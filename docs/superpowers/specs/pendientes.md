...
**Última actualización:** 2026-08-04, tras la revisión adversarial de rama de `feat/worker-y-operacion-web`
...
- **`apps/web/next.config.ts` no declara `serverExternalPackages`, así que webpack empaqueta el conector entero dentro del bundle del servidor.** La lista por omisión de Next solo trata como externo a `pg`, de lo que toca esta ruta; `@google-cloud/cloud-sql-connector`, `google-auth-library`, `gaxios` y `@googleapis/sqladmin` quedan adentro del bundle. Que `pnpm --filter @gc/web build` compile en verde no prueba que ese conector empaquetado funcione en Vercel: la prueba de humo que sí conectó contra la instancia real corrió con su propio proyecto de Node, sin pasar por webpack, así que no ejercitó este bundle en absoluto. La salida, si hace falta, es declarar esos cuatro paquetes en `serverExternalPackages` para que Next los deje como `require` normal en vez de empaquetarlos. Queda sin tocar a propósito: es información para quien despliegue, no un defecto que este bloque tenga que cerrar.
