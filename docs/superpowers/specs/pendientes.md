...
- **No hay forma de cerrar sesión desde la app.** `NextAuth(...)` en `apps/web/src/auth.ts` exporta `signOut`, pero ninguna pantalla lo llama: no existe botón ni ruta de salida. Hoy la única forma de terminar una sesión es esperar a que expire el JWT. No es alcance de esta rama construir esa pantalla; queda anotado para cuando se decida agregarla.
