...
- **`pg-connection-string` va a advertir en producción.** Parsea `sslmode` pero, para todo valor que no sea `verify-full` —`require`, `prefer`, `verify-ca`—, emite por `process.emitWarning` una `SECURITY WARNING` que avisa que esos modos hoy se tratan como alias de `verify-full` y que en `pg-connection-string@3.0.0` van a adoptar la semántica real de `libpq`, más débil. El "una vez por proceso" no lo da `process.emitWarning` —que no deduplica nada— sino una bandera propia del módulo (`deprecatedSslModeWarning.warned`, en `pg-connection-string/index.js`): la función se protege a sí misma para no repetir el aviso en cada `parse()`. Se confirmó corriendo `parse()` directo desde `node_modules/.pnpm/pg-connection-string@2.14.0` con `sslmode=require`. `postgres-js`, el driver anterior, no hacía ruido con `sslmode=require`. Hoy no se ve porque la URL local de Docker (`DATABASE_URL` en el `.env` de la raíz) no lleva `sslmode` en absoluto. Va a aparecer en cuanto la cadena de conexión de producción contra Cloud SQL sí lo lleve. No es un error —es ruido esperado del driver nuevo— pero conviene que quien conecte contra Cloud SQL no lo confunda con una falla real, y que sepa que fijar `sslmode=verify-full` explícito lo hace desaparecer.
