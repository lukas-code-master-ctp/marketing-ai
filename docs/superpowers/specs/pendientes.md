...
- `packages/brand/src/repositorio.test.ts` › «guardar el perfil registra quién lo hizo» verifica la autoría con `db.select(...).from(esquema.brandProfiles).where(eq(esquema.brandProfiles.version, version))` — selecciona la fila por `version` sin filtrar además por `brandId`. Con la siembra actual, una sola marca por corrida de prueba, `version` alcanza para identificar la fila sin ambigüedad. Si la siembra llegara a crecer a dos marcas dentro de la misma prueba, dos filas podrían compartir `version` y la aserción pasaría contra una fila que no es necesariamente la que se acaba de guardar, sin que nada lo señale. No es una prueba que no pueda fallar hoy —si `guardarPerfil` dejara de registrar `createdBy`, sí se pone roja—, pero la ambigüedad queda latente para quien amplíe la siembra sin revisar esta consulta.
